...
## Удаление фона + замена на «Африку» (горизонтальный PNG)

В проект добавлена отдельная страница для обработки фотографий:

- Запустите сервер (см. выше) и откройте: `http://localhost:8080/bg.html`
- Нажмите **«Выбрать фото»** → дождитесь статуса «Готово»
- Нажмите **«Скачать PNG (горизонтальное)»**

Примечание: обработка использует MediaPipe Selfie Segmentation и требует интернет-доступа к CDN при первом запуске.

...
async function startCamera() {
  // запускаем медиапоток после клика (жест пользователя)
  ...
  // autoplay на iOS требует muted+playsinline (есть в разметке)
...
async function initAR() {
  ...
  // Важно: путь к ПАПКЕ маркера и отдельно имя
  ...
  document.addEventListener("containerEvent", () => {
    ...
    // Привязываем объект к маркеру: add(object, markerName, visibilityWhenLost)
...
// Старт только по клику — это критично для разрешений камеры
startBtn.addEventListener("click", async () => {
  try {
    ...
    console.error(err);
    setStatus("Ошибка запуска. Проверь HTTPS и разрешения камеры.");
  }
});

// Переключение камеры: пересоздаём медиапоток
